...
export default function Wallet(props) {
    // Add onGameOver and events props
    const { gameStatus, onBetChange, onGameOver, events} = props;
  ...
  // Use useEffect to handle the different game outcomes
  useEffect(() => {
    switch (gameStatus) {
  ...
  // Create a function checkIfBroke that checks if the wallet amount is less than or equal to 0
  ...
  // Add a useEffect to reset the bet amount and clear the input field when the game is over
  useEffect(() => {
    if (gameStatus === events.bet) {
